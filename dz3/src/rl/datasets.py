"""Dataset generation utilities and CLI for train/val JSONL files."""
...
def build_val_datasets(
    ...
    val_sets: dict[int, list[Data]] = {}
    for difficulty in difficulties:
        val_sets[int(difficulty)] = env.generate(
...
def flatten_by_difficulty(by_difficulty: dict[int, list[Data]]) -> list[Data]:
    items: list[Data] = []
    for difficulty in sorted(by_difficulty):
        items.extend(by_difficulty[difficulty])
    return items
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate train/val datasets for modular arithmetic RL task"
    )
    ...
    parser.add_argument("--make-val", action="store_true", help="Generate val datasets")
    parser.add_argument(
        "--print-summary",
        action="store_true",
        help="Print difficulty distributions",
    )
    args = parser.parse_args()

    config = load_config(args.config, DEFAULT_DATA_CONFIG)
    make_train = args.make_train or (not args.make_train and not args.make_val)
    make_val = args.make_val or (not args.make_train and not args.make_val)
    ...
    if make_val:
        val_cfg = config["val"]
        val_sets = build_val_datasets(
            env=env,
            difficulties=val_cfg["difficulties"],
            questions_per_difficulty=int(val_cfg["questions_per_difficulty"]),
            seed_base=int(val_cfg["seed_base"]),
            max_attempts=int(val_cfg["max_attempts"]),
        )

        val_dir = Path(config["output"]["val_dir"])
        val_dir.mkdir(parents=True, exist_ok=True)
        for difficulty, items in sorted(val_sets.items()):
            path = val_dir / f"difficulty_{difficulty}.jsonl"
            save_jsonl_dataset(items, path)
            print(f"Saved val dataset to {path} ({len(items)} rows)")

        val_flat_path = Path(config["output"]["val_path"])
        val_items = flatten_by_difficulty(val_sets)
        save_jsonl_dataset(val_items, val_flat_path)
        print(f"Saved merged val dataset to {val_flat_path} ({len(val_items)} rows)")
        if args.print_summary:
            print("Val difficulty distribution:", summarize_dataset(val_items))
