"""GRPO training entrypoint for Qwen2.5-0.5B-Instruct with Unsloth."""
...
def _run_dry(config: dict[str, Any]) -> None:
    ...
    curve_path = Path(config["data"]["curves_dir"]) / "grpo_dry_run.jsonl"
    curve_path.parent.mkdir(parents=True, exist_ok=True)
    curve_path.write_text(
        json.dumps({"step": 0, "reward_mean": avg_reward, "entropy": None, "gen_len_mean": None})
        + "\n",
        encoding="utf-8",
    )
...
def _load_training_modules() -> tuple[Any, Any, Any, Any]:
    try:
        ...
        raise RuntimeError(
            ...
            "Install compatible unsloth/unsloth_zoo + datasets + trl."
        ) from exc
...
def _save_curves(trainer: Any, curves_path: Path) -> None:
    ...
    for row in getattr(trainer.state, "log_history", []):
        record = {
            "step": row.get("step"),
            "loss": row.get("loss"),
            "reward_mean": row.get("reward"),
            "entropy": row.get("entropy"),
            "gen_len_mean": row.get("completion_length"),
        }
...
def _train(config: dict[str, Any]) -> None:
    ...
    model_name = str(train_cfg.get("init_model_path") or model_cfg["base_model"])
    ...
    model_out = Path(config["models"]["grpo"])
    model_out.mkdir(parents=True, exist_ok=True)

    if hasattr(model, "save_pretrained_merged"):
        model.save_pretrained_merged(str(model_out), tokenizer, save_method="merged_16bit")
    else:
        model.save_pretrained(str(model_out))
        tokenizer.save_pretrained(str(model_out))

    curves_path = Path(config["data"]["curves_dir"]) / "grpo_train.jsonl"
    _save_curves(trainer, curves_path)
    print(f"Training completed. Saved model to {model_out}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Train GRPO for modular arithmetic environment")
    parser.add_argument("--config", default="configs/train.yaml", help="Path to training config")
    parser.add_argument("--dry-run", action="store_true", help="Run smoke check without model")
